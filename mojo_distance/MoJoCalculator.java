...
public class MoJoCalculator {
    ...
    public double[] mojofm() {
        ...
        /* tag assigment */
        tagAssignment("it.unisa.sesa.MoJo");
        ...
        double mojo =mojofmValue(cardinalitiesInB, numberOfObjectsInA, cost); 
            ...
            if (grouptags[j] != null)
            {
                for (int i = 0; i < m; i++)
                {
                    if (i != j && grouptags[j].objectList.elementAt(i).size() > 0)
                    {
                        ...
                        {
                            grouptags[i] = new it.unisa.sesa.Cluster(newClusterIndex++, l, m); // create a new group
...
*/
    ...
    private long calculateCost() {
        ...
        long totalCost = 0; /* total cost of it.unisa.sesa.MoJo */
